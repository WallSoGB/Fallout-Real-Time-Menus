...
*	 on using SCRIPT_PARAMETER_Array to accept array arguments directly.
*
...
*	 as SCRIPT_PARAMETER_Array. The argument can be extracted at run-time to an Array*, e.g.:
*		Array* arr;
 ...
 *	To pass a float, it must be cast like so: *(uint32_t*)&myFloat.
...
struct NVSEScriptInterface
{
	...
	bool	(*ExtractArgsEx)(SCRIPT_PARAMETER* apParameters, char* apCompiledParams, uint32_t* auiOffset, Script* apScript, ScriptLocals* apScriptLocals, ...);
	bool	(*ExtractFormatStringArgs)(uint32_t fmtStringPos, char* buffer, SCRIPT_PARAMETER* paramInfo, void* scriptDataIn,
 ...
 *
 *	    static SCRIPT_PARAMETER_TYPE s_MyEventParams[] = { SCRIPT_PARAMETER_TYPE::eParamType_AnyForm, SCRIPT_PARAMETER_TYPE::eParamType_String };
 *
...
{
	typedef void (*NativeEventHandler)(TESObjectREFR* apRef, void* apParams);

	// Mostly used for filtering information.
	enum SCRIPT_PARAMETER_TYPE : uint8_t
	{
		eParamType_Float = 0,
		eParamType_Int,
		eParamType_String,
	...
	static [[nodiscard]] bool IsFormParam(SCRIPT_PARAMETER_TYPE pType)
	{
		return pType == eParamType_RefVar || pType == eParamType_Reference || pType == eParamType_BaseForm
			|| pType == eParamType_Anything;
	}
	static [[nodiscard]] bool IsPtrParam(SCRIPT_PARAMETER_TYPE pType)
	{
		return (pType >= eParamType_FloatPtr) && (pType <= eParamType_BaseFormPtr);
	}
	// Gets the regular non-ptr version of the param
	static [[nodiscard]] SCRIPT_PARAMETER_TYPE GetNonPtrParamType(SCRIPT_PARAMETER_TYPE pType)
	{
		if (IsPtrParam(pType))
		{
			return static_cast<SCRIPT_PARAMETER_TYPE>(pType - 9);
		}
		return pType;
	}
	...
	bool (*RegisterEvent)(const char* name, uint8_t numParams, SCRIPT_PARAMETER_TYPE* paramTypes, EventFlags flags);
	...
	bool (*RegisterEventWithAlias)(const char* name, const char* alias, uint8_t numParams, SCRIPT_PARAMETER_TYPE* paramTypes, EventFlags flags);
...
struct ExpressionEvaluatorUtils
{
	...
	uint8_t(__fastcall* ScriptTokenGetType)(PluginScriptToken* scrToken);
	...
	SCRIPT_LOCAL* (__fastcall* ScriptTokenGetScriptVar)(PluginScriptToken* scrToken);
...
struct PluginScriptToken
{
	...
	bool CanConvertTo(uint8_t toType)
	{
